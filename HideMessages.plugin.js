 ...
 * @authorLink https://github.com/OsthatoAlfakyn
 * @description Fork of CollapseEmbeds by Zerthox that remembers collapsed attachments.
 * @source https://github.com/OsthatoAlfakyn/BetterDiscord-HideMessages
**/
...
const deletesave = (key) => BdApi.Data.delete(getMeta().name, key);
...
const queryTreeForMessage = (node) => {
    const worklist = [node].flat();
    while (worklist.length !== 0) {
        const node = worklist.shift();
        if (node?.message?.id) {
        ...
        if (node?.children) {
            worklist.push(...[node.children].flat());
        }
        if (node?.props) {
            worklist.push(...[node.props].flat());
        }
        if (node?.mediaAttachments) {
            worklist.push(...[node.mediaAttachments].flat());
        }
    }
}
...
const Hider = ({ placeholders, type, children }) => {
    const [shown, setShown] = React.useState(load(queryTreeForMessage(children)) ? false : !Settings.current.hideByDefault);
    ...
    return (React.createElement(Flex, { align: Flex.Align.CENTER, className: classNames(styles.container, styles[type], shown ? styles.expanded : styles.collapsed) },
        ...
        React.createElement(Clickable, { className: styles.hideButton, onClick: () => { setShown(!shown); shown && queryTreeForMessage(children) ? save(queryTreeForMessage(children), true) : deletesave(queryTreeForMessage(children)) } },
